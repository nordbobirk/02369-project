      ...
      <div className="flex sticky top-0 bg-card z-10 border-b mb-2 sm:mb-3">
        ...
        {headerDays.map((date, i) => (
          <div
            key={date.toString()}
            className={cn(
              'text-center flex-1 gap-0.5 sm:gap-1 pb-1 sm:pb-2 text-[10px] sm:text-sm text-muted-foreground flex flex-col sm:flex-row items-center justify-center',
        ...
        <div className="grid grid-cols-7 flex-1 min-w-0">
          {weekDates.map((hours, i) => {
            return (
              <div
                className={cn(
                  'h-full text-xs sm:text-sm text-muted-foreground border-l first:border-l-0 min-w-0',
      ...
      <div className="grid grid-cols-7 gap-px sticky top-0 bg-background border-b">
        {weekDays.map((day, i) => (
          <div
            key={day}
            className={cn(
              'mb-1 sm:mb-2 text-center sm:text-right text-[10px] sm:text-sm text-muted-foreground sm:pr-2',
              [0, 6].includes(i) && 'text-muted-foreground/50'
            )}
          >
            {day}
      ...
      <div className="grid overflow-hidden -mt-px flex-1 auto-rows-fr p-px grid-cols-7 gap-px">
        {monthDates.map((_date) => {
          ...
          return (
            <div
              className={cn(
                'ring-1 p-1 sm:p-2 text-[10px] sm:text-sm text-muted-foreground ring-border overflow-auto',
            ...
            >
              <span
                className={cn(
                  'size-5 sm:size-6 text-[10px] sm:text-sm grid place-items-center rounded-full mb-0.5 sm:mb-1 sticky top-0',
              ...
              {currentEvents.map((event) => {
                return (
                  <div
                    key={event.id}
                    className="px-0.5 sm:px-1 rounded text-[10px] sm:text-sm flex items-center gap-0.5 sm:gap-1 mb-0.5"
                  >
                    <div
                      className={cn(
                        'shrink-0',
...
const CalendarYearView = () => {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-10 overflow-auto h-full p-2 sm:p-0">
      {months.map((days, i) => (
        <div key={days[0].toString()}>
          <span className="text-base sm:text-lg lg:text-xl">{i + 1}</span>

          <div className="grid grid-cols-7 gap-1 sm:gap-2 my-2 sm:my-3 lg:my-5">
            {weekDays.map((day) => (
              <div
                key={day}
          ...
          <div className="grid gap-x-1 sm:gap-x-2 text-center grid-cols-7 text-[10px] sm:text-xs tabular-nums">
            {days.map((_date) => {
              return (
                <div
                  key={_date.toString()}
